...
from app.models import ChunkMetadata, ChunkResult, JobStatus, JobStatusResponse, UploadChunkRequest
    ...
    try:
        ...
        if is_google_drive_url(url):
            try:
                ...
                raise ImportError("gdown package is required for Google Drive downloads. Install with: pip install gdown")
...
async def download_google_drive_file(url: str, output_dir: str, job_id: str, gdown_module) -> tuple:
    ...
    try:
        ...
        # Download the file
        gdown_module.download(f"https://drive.google.com/uc?id={file_id}", temp_file_path, quiet=False)
...
async def trigger_callback(job_id: str, callback_url: str):
    ...
    try:
        ...
        logger.error(f"Connection error for callback to {callback_url} for job {job_id}")
    except requests.exceptions.HTTPError as e:
        logger.error(f"HTTP error in callback for job {job_id}: {e.response.status_code} - {e.response.text}")
...
async def _retry_callback_with_simple_payload(job_id: str, callback_url: str, job_data: dict):
...
async def upload_and_chunk_endpoint(
    ...
    if urls:
        ...
        for url in urls:  # Directly iterate over the list
            try:
                file_path, original_filename = await download_file_from_url(url, temp_dir, job_id)
        ...
        for chunk_data in result:
            if isinstance(chunk_data, dict):
                ...
                if "metadata" in chunk_data and isinstance(chunk_data["metadata"], dict):
                    ...
                    chunk_id = chunk_data.get("id", f"chunk_{len(chunk_results) + 1:03d}")
                    metadata = ChunkMetadata(
                        page_num_int=[1],  # Default page
                        original_filename=chunk_data.get("metadata", {}).get("original_filename", "unknown"),
                        chunk_size=chunk_data.get("metadata", {}).get("chunk_size"),
                        chunk_overlap=chunk_data.get("metadata", {}).get("chunk_overlap")
                    )
...
async def cancel_job(job_id: str):
    ...
    if job["status"] in [JobStatus.COMPLETED, JobStatus.FAILED, JobStatus.CANCELLED]:
        ...
        logger.info(f"Cancel requested for job {job_id} which is already in {job['status']} state")
        return {
            "status": "cancelled", 
...
async def health_check():
    ...
    if settings.EMBEDDING_SERVICE_URL:
        try:
            ...
            embedding_status = "connected" if response.status_code < 500 else "unavailable"
        except Exception as e:
